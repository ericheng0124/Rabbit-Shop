...
// 获取购物车列表数据
const cartListData = ref<CartItem[]>([])
const getCartListData = async () => {
  const res = await getCartListAPI()
  cartListData.value = res.result
}
...
const onDeleteCart = (skuId: string) => {
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        // 后端删除单品
        await deleteCartListAPI({ ids: [skuId] })
        // 重新获取购物车列表
        getCartListData()
      }
    },
...
const selectedCartList = computed(() => {
  return cartListData.value.filter((item) => item.selected)
})
...
<template>
  <scroll-view scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
      <!-- 购物车列表 -->
      <view class="cart-list" v-if="cartListData.length">
        <!-- 优惠提示 -->
        <view class="tips">
        ...
        <uni-swipe-action>
          <!-- 滑动操作项 -->
          <uni-swipe-action-item v-for="item in cartListData" :key="item.id" class="cart-swipe">
            <!-- 商品信息 -->
            <view class="goods">
              <!-- 选中状态 -->
              <text
